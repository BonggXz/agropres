...
const DashboardHeader = memo(({ user }) => {
  ...
  return (
    <motion.header 
      ...
      className="relative overflow-hidden bg-gradient-to-r from-white to-gray-50 rounded-2xl shadow-xl border border-gray-100 mb-8"
    >
...
const StatusCard = memo(({ isOnline }) => (
  <motion.div 
    custom={1} 
    variants={cardVariants} 
    className="relative overflow-hidden bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl border border-gray-100"
  >
              ...
              <div className="p-4 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl border border-yellow-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="font-semibold text-gray-700">Lampu UV</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <span className="px-3 py-1 bg-white text-gray-800 font-medium rounded-lg shadow-sm">
                      {deviceData?.relay_schedules?.uv_light?.on_time || '--:--'}
                    </span>
                    <span className="text-gray-400">—</span>
                    <span className="px-3 py-1 bg-white text-gray-800 font-medium rounded-lg shadow-sm">
                      {deviceData?.relay_schedules?.uv_light?.off_time || '--:--'}
                    </span>
                  </div>
                </div>
              </div>
              <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="font-semibold text-gray-700">Ultrasonik</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <span className="px-3 py-1 bg-white text-gray-800 font-medium rounded-lg shadow-sm">
                      {deviceData?.relay_schedules?.ultrasonic?.on_time || '--:--'}
                    </span>
                    <span className="text-gray-400">—</span>
                    <span className="px-3 py-1 bg-white text-gray-800 font-medium rounded-lg shadow-sm">
                      {deviceData?.relay_schedules?.ultrasonic?.off_time || '--:--'}
                    </span>
                  </div>
                </div>
              </div>
            ...
            >
              <div className="space-y-3">
                <label className="flex items-center space-x-2 text-sm font-semibold text-gray-700">
                  ...
                  <span>Jadwal Lampu UV</span>
                </label>
                <div className="flex items-center space-x-3">
                  <input 
                    type="time" 
                    value={relayScheduleForm.uv_light.on_time} 
                    onChange={e => setRelayScheduleForm({
                      ...relayScheduleForm, 
                      uv_light: {...relayScheduleForm.uv_light, on_time: e.target.value}
                    })} 
                    className="flex-1 px-4 py-2 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-all duration-200"
                  />
                  ...
                  <input 
                    ...
                    onChange={e => setRelayScheduleForm({
                      ...relayScheduleForm, 
                      uv_light: {...relayScheduleForm.uv_light, off_time: e.target.value}
                    })} 
                    className="flex-1 px-4 py-2 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-all duration-200"
                  />
                </div>
              ...
              <div className="space-y-3">
                <label className="flex items-center space-x-2 text-sm font-semibold text-gray-700">
                  <FaVolumeUp className="w-4 h-4 text-blue-600" />
                  <span>Jadwal Suara Ultrasonik</span>
                </label>
                <div className="flex items-center space-x-3">
                  <input 
                    type="time" 
                    ...
                    onChange={e => setRelayScheduleForm({
                      ...relayScheduleForm, 
                      ultrasonic: {...relayScheduleForm.ultrasonic, on_time: e.target.value}
                    })} 
                    className="flex-1 px-4 py-2 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                  />
                  <span className="text-gray-400 font-bold">—</span>
                  <input 
                    type="time" 
                    ...
                    onChange={e => setRelayScheduleForm({
                      ...relayScheduleForm, 
                      ultrasonic: {...relayScheduleForm.ultrasonic, off_time: e.target.value}
                    })} 
                    className="flex-1 px-4 py-2 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                  />
                </div>
...
const WhatsAppSchedulerCard = memo(({ user, userData }) => {
  ...
  const handleAddSchedule = async (e) => {
    ...
    try {
      ...
      setNewSchedule({ 
        ...
        time: '', 
        note: '', 
        targetNumber: userData?.whatsapp_number || '', 
        message: '' 
      });
  ...
  // Fixed WhatsApp scheduler logic
  const checkSchedules = useCallback(() => {
    ...
    Object.entries(schedules).forEach(([id, schedule]) => {
      if (schedule.status === 'active') {
        ...
        // Check if the schedule time has passed (with 1 minute tolerance)
        if (now >= scheduleTime && (now - scheduleTime) <= 60000) {
          // Send WhatsApp message
          const whatsappUrl = `https://api.whatsapp.com/send?phone=${schedule.targetNumber}&text=${encodeURIComponent(schedule.message)}`;
          window.open(whatsappUrl, '_blank');
          
          // Update schedule status to 'sent'
          update(ref(db, `users/${user.uid}/pestisida_schedules/${id}`), {
            status: 'sent',
          ...
          Swal.fire({
            ...
            text: `Pesan "${schedule.note}" telah dikirim ke ${schedule.targetNumber}`,
            icon: 'success',
            timer: 3000,
            showConfirmButton: false
          });
  ...
  return (
    <motion.div 
      ...
      className="relative overflow-hidden bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl border border-gray-100"
    >
      <div className="absolute top-0 left-0 w-32 h-32 bg-gradient-to-br from-green-500/10 to-emerald-500/10 rounded-full -translate-y-16 -translate-x-16 opacity-50"></div>
      <div className="relative p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl">
              <BsCalendar3 className="w-5 h-5 text-white" />
            </div>
            <h2 className="text-xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">
              Jadwal Pestisida
            </h2>
        ...
        <form onSubmit={handleAddSchedule} className="space-y-4 mb-6">
          ...
          <input 
            type="text" 
            placeholder="Catatan (contoh: Penyemprotan Pestisida Area A)" 
          ...
          <textarea 
            ...
            className="w-full px-4 py-2 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-200 h-20 resize-none"
            required
          />
          <motion.button 
        ...
        <div className="space-y-3 max-h-64 overflow-y-auto">
          <AnimatePresence>
            {Object.entries(schedules).map(([id, schedule], index) => (
              <motion.div 
                key={id}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <p className="font-semibold text-gray-800">{schedule.note}</p>
                    <p className="text-sm text-gray-600">Ke: {schedule.targetNumber}</p>
                    <p className="text-sm text-gray-500 mt-1 italic">"{schedule.message}"</p>
                  </div>
                  <motion.button 
                    whileHover={{ scale: 1.1 }} 
                    whileTap={{ scale: 0.9 }}
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const deviceRef = ref(db, `devices/${userData.device_id}`);
  ...
  // Load relay modes from device data
  useEffect(() => {
    if (deviceData?.relay_modes) {
      setUvMode(deviceData.relay_modes.uv_light || 'manual');
      setUltrasonicMode(deviceData.relay_modes.ultrasonic || 'manual');
    }
  }, [deviceData]);

  // Load relay schedules form
  useEffect(() => {
    if (deviceData?.relay_schedules) {
      setRelayScheduleForm({
        uv_light: {
          on_time: deviceData.relay_schedules.uv_light?.on_time || '',
          off_time: deviceData.relay_schedules.uv_light?.off_time || ''
        },
        ultrasonic: {
          on_time: deviceData.relay_schedules.ultrasonic?.on_time || '',
          off_time: deviceData.relay_schedules.ultrasonic?.off_time || ''
        }
      });
    }
  }, [deviceData]);
  ...
  const handleModeChange = async (device, mode) => {
    if (!userData?.device_id) return;
    try {
      await set(ref(db, `devices/${userData.device_id}/relay_modes/${device}`), mode);
      if (device === 'uv_light') setUvMode(mode);
      if (device === 'ultrasonic') setUltrasonicMode(mode);
      
      Swal.fire({
        title: 'Mode Berhasil Diubah!',
        text: `${device === 'uv_light' ? 'Lampu UV' : 'Ultrasonik'} sekarang dalam mode ${mode === 'auto' ? 'otomatis' : 'manual'}`,
        icon: 'success',
        timer: 2000,
        showConfirmButton: false
      });
    } catch (error) {
      Swal.fire('Error!', 'Gagal mengubah mode perangkat.', 'error');
    }
  };
  ...
  const handleManualToggle = async (device, state) => {
    if (!userData?.device_id) return;
    try {
      await set(ref(db, `devices/${userData.device_id}/controls/${device}`), state);
      Swal.fire({
        title: `${device === 'uv_light' ? 'Lampu UV' : 'Ultrasonik'} ${state ? 'Dinyalakan' : 'Dimatikan'}!`,
        icon: 'success',
        timer: 1500,
        showConfirmButton: false
      });
    } catch (error) {
      Swal.fire('Error!', 'Gagal mengontrol perangkat.', 'error');
    }
  };
  ...
  const handleRelayScheduleSave = async () => {
    if (!userData?.device_id) return;
    try {
      await set(ref(db, `devices/${userData.device_id}/relay_schedules`), relayScheduleForm);
      Swal.fire({
        title: 'Jadwal Tersimpan!',
        text: 'Jadwal otomatis perangkat berhasil diperbarui.',
        icon: 'success',
        timer: 2000,
        showConfirmButton: false
      });
